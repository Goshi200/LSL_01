...
public class ReadEEG : ADoubleInlet
{
    // ■記録用変数
    ...
    private void FixedUpdate()
    {
        ...
        for (int i = 0; i < 14; i++)
        {
            SetPlot($"test{i}", CreateConsecutiveArray(len), ExtractArrayFromList(eeg_list, 4+i, len), new Vector2(-30, 40-7*i), new Vector2(100, 12), new Color(0, 0, 0), 0.2f);
        }
    }
    // ◆プロセス
    ...
    // ◆グラフ表示
    private void SetPlot(string line_obj_key, double[] x, double[] y,Vector2 anchor, Vector2 size, Color color, float weight=0.1f)
    {
        GameObject obj;
        if (line_dictionary.ContainsKey(line_obj_key) == true) { obj = line_dictionary[line_obj_key]; }
        else
        {
            obj = Instantiate(line_obj);
        ...
        line_renderer.startWidth = weight;
        line_renderer.endWidth = weight;
